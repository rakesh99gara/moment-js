const MomentAnalyse = require("./MomentAnalyse");
const LocaleSupport = require("./LanguageSupport");
const MomentArrangement = require("./MomentArrangement");
...
class Moment {
  ...
  parseTime() {
    this.momentParser = new MomentAnalyse(
      this.date,
      this.format,
      this.locale,
      this.isStrict
  ...
  Format(givenFormat) {
    return new MomentArrangement(
      this.momentContainer,
      givenFormat,
      this.locale
    ).formatTime();
  }
  ...
  from(b) {
    let momentComparitor = new MomentComparitor(
      this.momentContainer.moment,
      b.momentContainer.moment
    );
  ...
  fromNow() {
    let curr_moment = new MomentAnalyse(new Date(), null, this.locale);
    let momentComparitor = new MomentComparitor(
      this.momentContainer.moment,
      curr_moment.parseMoment().moment
    );
    let res = momentComparitor.compare();
    if (res[0] == 0) {
      return "same";
  ...
  calendar(rt) {
    ...
    let momentComparitor = new MomentComparitor(
      this.momentContainer.moment,
      referenceTime.momentContainer.moment
    );
  ...
  isBetween(rt1, rt2, units) {
    let referenceTime1 = rt1;
    let referenceTime2 = rt2;
    let momentComparitor1 = new MomentComparitor(
      this.momentContainer.moment,
      referenceTime1.momentContainer.moment
    );
    let momentComparitor2 = new MomentComparitor(
      this.momentContainer.moment,
      referenceTime2.momentContainer.moment
    );
    if (
      (momentComparitor1.getDiff(units) < 0 &&
        momentComparitor2.getDiff(units) > 0) ||
      (momentComparitor1.getDiff(units) > 0 &&
        momentComparitor2.getDiff(units) < 0)
    ) {
      return true;
