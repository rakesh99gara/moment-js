...
class MomentComparator {
  constructor(a, b) {
    ...
    //add offset
    let offsetA =
      (this.a["ZZ"].substring(0, 1) == "-" ? -1 : 1) *
      parseInt(
        this.a["ZZ"].substring(1, 2) * 100 +
          parseInt(this.a["ZZ"].substring(2, this.a["ZZ"].length))
      ) *
      60 *
      1000;
    let offsetB =
      (this.a["ZZ"].substring(0, 1) == "-" ? -1 : 1) *
      parseInt(
        this.b["ZZ"].substring(1, 2) * 100 +
          parseInt(this.b["ZZ"].substring(2, this.b["ZZ"].length))
      ) *
      60 *
      1000;
  ...
  calendar() {
    let diffIndays =
      Math.floor(this.b_ms_gmt / (1000 * 60 * 60 * 24)) -
      Math.floor(this.a_ms_gmt / (1000 * 60 * 60 * 24));
    return diffIndays;
  }
  ...
  getDiff(units) {
    ...
    return (
      Math.floor(this.b_ms_gmt / unitstoValue[units]) -
      Math.floor(this.a_ms_gmt / unitstoValue[units])
    );
  }
}
