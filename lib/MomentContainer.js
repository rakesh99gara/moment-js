...
class MomentContainer {
  constructor(timeStamp, offset, locale) {
    ...
    this.moment["Z"] =
      offset.substring(0, 2) + ":" + offset.substring(2, offset.length);

    //fractional seconds
    let msLength = milliSec.toString().length;
    // console.log(
    //   "MsLength : " + "00".substring(0, 3 - msLength) + milliSec.toString()
    // );
    this.moment["SSS"] = "00".substring(0, 3 - msLength) + milliSec.toString();
    this.moment["SS"] =
      "0".substring(0, 2 - (milliSec / 10).toString().length) +
      (milliSec / 10).toString();
    this.moment["S"] = (milliSec / 100).toString();

    //seconds
    this.moment["ss"] =
      "0".substring(0, 2 - sec.toString().length) + sec.toString();
    this.moment["s"] = sec.toString();

    //minutes
    this.moment["mm"] =
      "0".substring(0, 2 - min.toString().length) + min.toString();
    this.moment["m"] = min.toString();

    //hours
    let kHour = hour == 0 ? 24 : hour;
    // console.log("0".substring(0, 2 - kHour.toString().length) + kHour.toString());
    this.moment["kk"] =
      "0".substring(0, 2 - kHour.toString().length) + kHour.toString();
    this.moment["k"] = kHour.toString();
    let usualHour = kHour >= 12 ? kHour - 12 : kHour;
    this.moment["hh"] =
      "0".substring(0, 2 - usualHour.toString().length) + usualHour.toString();
    this.moment["h"] = usualHour.toString();
    this.moment["HH"] =
      "0".substring(0, 2 - hour.toString().length) + hour.toString();
    ...
    this.moment["GGGG"] =
      "0000".substring(0, 4 - weekYear.toString().length) + weekYear.toString();
    this.moment["GG"] = this.moment["GGGG"].substring(
      weekYear.toString().length - 2,
      weekYear.toString().length
    );

    //week year
    this.moment["gggg"] =
      "0000".substring(0, 4 - year.toString().length) + year.toString();
    this.moment["gg"] = this.moment["gggg"].substring(
    ...
    if (weekYear > year) {
      ...
      let daysRemaining = MomentContainer.convert(
        (timeStamp - yearStart.getTime()) / 1000 -
          MomentContainer.convert(daysInFirstWeek, "days", "seconds"),
        "seconds",
        "days"
      );
    ...
    this.moment["WW"] =
      "0".substring(0, 2 - weekOfYearIso.toString().length) +
      weekOfYearIso.toString();
    this.moment["Wo"] =
      weekOfYearIso.toString() + MomentContainer.getPrefix(weekOfYearIso);
    ...
    this.moment["ww"] =
      "0".substring(0, 2 - weekOfYear.toString().length) +
      weekOfYear.toString();
    this.moment["wo"] =
      weekOfYear.toString() + MomentContainer.getPrefix(weekOfYear);
    ...
    this.moment["DDDD"] =
      "00".substring(0, 3 - dayOfYear.toString().length) + dayOfYear.toString();
    this.moment["DDDo"] =
      dayOfYear.toString() + MomentContainer.getPrefix(dayOfYear);
    ...
    this.moment["DD"] =
      "0".substring(0, 2 - dayOfMonth.toString().length) +
      dayOfMonth.toString();
    this.moment["Do"] =
      dayOfMonth.toString() + MomentContainer.getPrefix(dayOfMonth);
    ...
    this.moment["MM"] =
      "0".substring(0, 2 - monthToShow.toString().length) +
      monthToShow.toString();
    this.moment["Mo"] =
      monthToShow.toString() + MomentContainer.getPrefix(monthToShow);
